...
class InstagramProvider(ChannelProvider):
    def send(self, channel: Channel, message: Message) -> SendResult:
        cfg = channel.config
        url = f"{IG_BASE}/{cfg.ig_user_id}/messages"
        ...
        try:
            ...
            logger.error("Instagram request failed: %s", e)
            return SendResult(False, error_code="NETWORK", error_message=str(e))
        except (ValueError, KeyError) as e:
            logger.error("Instagram parse error: %s", e)
    ...
    def validate(self, channel: Channel) -> bool:
        ...
        try:
            with httpx.Client(timeout=10) as c:
                r = c.get(
                    f"{IG_BASE}/{cfg.ig_user_id}",
                    headers={"Authorization": f"Bearer {cfg.fb_page_access_token}"},
                )
            return r.status_code == 200
        except httpx.RequestError as e:
            logger.warning("Instagram validate error: %s", e)
            return False
